function [X, fX, i] = optimizeHyp(X, length, infer, cov, lik, x, y)
...
% [infer, cov, lik, x, y] = varargin{:};

i = 0;                                            % zero the run length counter
% ls_failed = 0;                             % no previous line search has failed        
[~,f0,df0] = infer(X, cov, lik, x, y); % get function value and gradient

Z = X; X = unwrap(X); df0 = unwrap(df0); % unwrap to retract cov
fprintf('%s %6i;  Value %4.6e\r', S, i, f0);
fX = f0;
i = i + 1; %+ (length<0);                                            % count epochs?!
s = -df0; d0 = -s'*s;           % initial search direction (steepest) and slope
x3 = 1/(1-d0);                                  % initial step is red/(|s|+1)
...
%   if length>0, M = MAX; else M = min(MAX, -length-i); end
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        M = M - 1; i = i + 1;   
        [~,f3,df3] = infer(rewrap(Z,X+x3*s),cov, lik, x, y);
        df3 = unwrap(df3);
